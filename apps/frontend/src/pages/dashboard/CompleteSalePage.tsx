...
import { DEFAULT_RECEIPT_TYPES, findReceiptTypeByAfipCode } from '@/lib/constants/afipCodes'
...
export default function CompleteSalePage() {
  ...
  const fetchReceiptTypes = useCallback(async () => {
    try {
      ...
      const apiData = Array.isArray(response) ? response : 
                     Array.isArray(response?.data?.data) ? response.data.data :
                     Array.isArray(response?.data) ? response.data : []
      const mappedData = apiData.map((item: any): ReceiptType => ({ 
        id: item.id, 
  ...
  const handleConfirmSale = useCallback(async () => {
    ...
    try {
      ...
      const saleData: SaleData = {
        ...
        total_iva: totalIva,
        total: total,
        total_discount: Math.max(0, totalItemDiscount + globalDiscountAmount),
  ...
  const diff = useMemo(() => roundToTwoDecimals(total - paid), [total, paid])
