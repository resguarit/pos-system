...
import type { Option } from '@/components/ui/multi-select-combobox';
...
export default function VentasPage() {
  ...
  useEffect(() => {
    ...
    fetchProducts();
  }, [request, debouncedProductQuery, productPage, selectedBranchIds, productFilterOpen]);

  const productSummary = useMemo(() => {
    if (!selectedProductIds.length) return "Todos";
    const first = productLabelMap[selectedProductIds[0]] || `Producto ${selectedProductIds[0]}`;
    if (selectedProductIds.length === 1) return first;
    return `${first} +${selectedProductIds.length - 1}`;
  }, [productLabelMap, selectedProductIds]);

  const toggleProduct = (value: string) => {
    if (selectedProductIds.includes(value)) {
      setSelectedProductIds(selectedProductIds.filter((v) => v !== value));
    } else {
                          ...
                          <Popover open={productFilterOpen} onOpenChange={setProductFilterOpen}>
                              <PopoverTrigger asChild>
                                ...
                                >
                                  ...
                                  <span className="ml-2 flex items-center gap-1 text-muted-foreground text-xs">
                                    <span className="truncate max-w-[90px]">{productSummary}</span>
                                    <ChevronDown className="h-3 w-3 opacity-70" />
                                  </span>
                                </Button>
                              </PopoverTrigger>
                              <PopoverContent className="w-80" style={{ maxHeight: 320, overflowY: 'auto' }}>
                                <div className="mb-2 text-xs text-muted-foreground">Selecciona productos</div>
                                <Input
                                  type="search"
                                ...
                                <div className="mt-2 grid gap-2">
                                  {productQuery.trim().length < 2 ? (
                                    <div className="text-xs text-muted-foreground">Escribi al menos 2 caracteres para buscar.</div>
                                  ) : productLoading && productPage === 1 ? (
                                    <div className="text-xs text-muted-foreground">Buscando productos...</div>
                                  ) : productOptions.length === 0 ? (
                                    <div className="text-xs text-muted-foreground">No se encontraron productos</div>
                                  ) : (
                                    productOptions.map((opt) => (
                                      <label key={opt.value} className="flex items-center gap-2 text-sm">
                                        <Checkbox
                                          checked={selectedProductIds.includes(opt.value)}
                                  ...
                                  {productHasMore && productQuery.trim().length >= 2 && (
                                    <Button
                                      variant="ghost"
                                      size="sm"
                                    ...
                                    >
                                      {productLoading ? "Cargando..." : "Cargar mas"}
                                    </Button>
                                  )}
                                </div>
                              </PopoverContent>
